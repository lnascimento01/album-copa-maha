...
export default function StickerForm({ albums, players, types, rarities, initialValues, submitLabel, submitUrl, method }: StickerFormProps) {
    ...
    const suggestCode = () => {
        const prefix = codePrefixByType[form.data.type] ?? 'AAPH';
    ...
    return (
        <form onSubmit={submit} className="album-paper space-y-4 p-4">
            <div className="grid gap-3 md:grid-cols-2">
                ...
                <div>
                    <label className="admin-filter-label">Jogador vinculado (opcional)</label>
                    <div className="flex gap-2">
                        <select className="mt-1 w-full rounded-sm border border-border bg-card px-2 py-2 text-sm" value={form.data.player_id} onChange={(event) => form.setData('player_id', Number(event.target.value) || '')}>
                            <option value="">Sem vínculo</option>
                            {playerOptions.map((player) => <option key={player.id} value={player.id}>{player.name}</option>)}
                        </select>
                        <button type="button" onClick={syncTitleFromPlayer} className="app-link-chip mt-1 !px-2">
                            Usar nome
                        </button>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="flex gap-2">
                        <input className="mt-1 w-full rounded-sm border border-border bg-card px-2 py-2 text-sm font-mono" value={form.data.code} onChange={(event) => form.setData('code', event.target.value)} placeholder="AAPH-001" />
                        <button type="button" onClick={suggestCode} className="app-link-chip mt-1 !px-2">
                            Sugerir
                        </button>
                    </div>
                    <p className="mt-1 text-[11px] text-dim">Exemplos: AAPH-001, GK-001, MOM-001.</p>
                ...
                <div>
                    ...
                    <input className="mt-1 w-full rounded-sm border border-border bg-card px-2 py-2 text-sm" value={form.data.title} onChange={(event) => form.setData('title', event.target.value)} placeholder="Ex.: Atleta AAPH 01" />
                    {form.errors.title ? <p className="mt-1 text-xs text-red-600">{form.errors.title}</p> : null}
                </div>

                <div>
                    <label className="admin-filter-label">Subtítulo</label>
                    <input className="mt-1 w-full rounded-sm border border-border bg-card px-2 py-2 text-sm" value={form.data.subtitle} onChange={(event) => form.setData('subtitle', event.target.value)} placeholder="Ex.: Ponta AAPH" />
                </div>
                <div>
                    <label className="admin-filter-label">Imagem (URL/caminho)</label>
                    <input className="mt-1 w-full rounded-sm border border-border bg-card px-2 py-2 text-sm" value={form.data.image_path} onChange={(event) => form.setData('image_path', event.target.value)} placeholder="https://... ou /storage/..." />
                    <p className="mt-1 text-[11px] text-dim">Nesta etapa o campo é textual com preview simples.</p>
                </div>
                ...
                <div>
                    <label className="admin-filter-label">Raridade</label>
                    <select className="mt-1 w-full rounded-sm border border-border bg-card px-2 py-2 text-sm" value={form.data.rarity} onChange={(event) => form.setData('rarity', event.target.value)}>
                        {rarities.map((item) => <option key={item} value={item}>{item}</option>)}
                    </select>
                </div>

                <div>
                    <label className="admin-filter-label">Ordem no álbum</label>
            ...
            {hasImagePreview ? (
                <div className="rounded-sm border border-border bg-muted/70 p-3">
                    <div className="admin-filter-label">Preview da figurinha</div>
                    <div className="mt-2 flex items-start gap-3">
                        <img src={form.data.image_path} alt="Preview da figurinha" className="h-40 w-28 rounded-sm border border-border object-cover" />
                        <div className="rounded-sm border border-border bg-card p-2 text-xs text-foreground">
                            ...
                            <div className="text-dim">{form.data.subtitle || 'Subtítulo'}</div>
                        </div>
                    </div>
                </div>
            ) : null}
            ...
            <div>
                ...
                <textarea className="mt-1 w-full rounded-sm border border-border bg-card px-2 py-2 text-sm" rows={4} value={form.data.description} onChange={(event) => form.setData('description', event.target.value)} placeholder="Texto de apoio para contexto da figurinha." />
            </div>
