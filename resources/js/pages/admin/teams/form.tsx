...
export default function TeamForm({ initialValues, submitLabel, submitUrl, method }: TeamFormProps) {
    ...
    return (
        <form onSubmit={submit} className="grid gap-3 rounded-sm border p-4 md:grid-cols-2">
            ...
            <div>
                <label className="text-xs uppercase text-muted-foreground">Sigla</label>
                <input className="mt-1 w-full rounded-sm border px-2 py-2 text-sm" value={form.data.short_name} onChange={(event) => form.setData('short_name', event.target.value)} />
            </div>
            <div>
                <label className="text-xs uppercase text-muted-foreground">Logo path</label>
                <input className="mt-1 w-full rounded-sm border px-2 py-2 text-sm" value={form.data.logo_path} onChange={(event) => form.setData('logo_path', event.target.value)} />
            </div>
            <div>
                <label className="text-xs uppercase text-muted-foreground">Cor primária</label>
                <input className="mt-1 w-full rounded-sm border px-2 py-2 text-sm" value={form.data.primary_color} onChange={(event) => form.setData('primary_color', event.target.value)} />
            </div>
            <div>
                <label className="text-xs uppercase text-muted-foreground">Cor secundária</label>
                <input className="mt-1 w-full rounded-sm border px-2 py-2 text-sm" value={form.data.secondary_color} onChange={(event) => form.setData('secondary_color', event.target.value)} />
            </div>
